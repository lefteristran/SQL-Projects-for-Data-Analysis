...
-- Create Project (ecommerce) Schema
...
CREATE TABLE ecommerce.campaigns_stage (
...
--#Insert data to ST
BULK INSERT ecommerce.campaigns_stage
...
CREATE TABLE ecommerce.campaigns_dim (
...
INSERT INTO ecommerce.campaigns_dim (campaign_id, channel, objective, start_date, end_date, target_segment, expected_uplift)
SELECT
    campaign_id,
    channel,
    objective,
    ...
    target_segment, 
    expected_uplift
FROM ecommerce.campaigns_stage

--#Drop ST campaigns
DROP TABLE ecommerce.campaigns_stage


--------------------------------------- Dataset Customers (7 Columns)

--#Create  DIMENSION table for customers
CREATE TABLE ecommerce.customers_dim (
...
BULK INSERT ecommerce.customers_dim
...
CREATE TABLE ecommerce.products_dim (
...
--#Insert data to  DIMENSION table
BULK INSERT ecommerce.products_dim
...
CREATE TABLE ecommerce.events_fact_stage (
...
FOREIGN KEY (customer_id) REFERENCES ecommerce.customers_dim(customer_id),
FOREIGN KEY (product_id) REFERENCES ecommerce.products_dim(product_id),
FOREIGN KEY (campaign_id) REFERENCES ecommerce.campaigns_dim(campaign_id))

--#Insert data to  FACT ST table
BULK INSERT ecommerce.events_fact_stage
...
CREATE TABLE ecommerce.events_fact (
event_id INT PRIMARY KEY NOT NULL,
...
INSERT INTO ecommerce.events_fact (
    event_id,
    events_timestamp,
    customer_id,
    session_id,
...
SELECT
    ...
    session_duration_sec,
    experiment_group
FROM ecommerce.events_fact_stage;

--#DROP events FACT ST
DROP TABLE ecommerce.events_fact_stage



-------------------------------------- Dataset Transactions (9 Columns)
--# CREATE FACT ST Table
CREATE TABLE ecommerce.transactions_fact_stage (
...
BULK INSERT ecommerce.transactions_fact_stage
...
CREATE TABLE ecommerce.transaction_fact (
transaction_id INT PRIMARY KEY NOT NULL,
...
INSERT INTO ecommerce.transaction_fact (
    transaction_id,
...
SELECT
    ...
    NULLIF(campaign_id, 0),
    refund_flag
FROM ecommerce.transactions_fact_stage


--#DROP transactions  FACT ST
DROP TABLE ecommerce.transactions_fact_stage
...
-------------- ecommerce.campaigns_dim | First Exploration

-- Check number of rows
SELECT COUNT(*) FROM ecommerce.campaigns_dim -- 50 rows

-- Quick check of top 10 rows - data - col
SELECT TOP 10 * FROM ecommerce.campaigns_dim  

-- Check Primary Key is Unique
SELECT campaign_id, COUNT(*)
FROM ecommerce.campaigns_dim 
...
FROM ecommerce.campaigns_dim                  -- no missing values
...
FROM ecommerce.campaigns_dim                                                                  -- first: 2021-01-21 , last: 2024 - 01 - 06. Campaings lasted for almost 3.5 years.
...
-------------- ecommerce.customers_dim | First Exploration

--Check first 10 rows
SELECT TOP 10 * FROM ecommerce.customers_dim  

-- Get total number of records
SELECT COUNT(*) FROM ecommerce.customers_dim --100k rows
...
FROM ecommerce.customers_dim    -- no missing data
...
FROM ecommerce.customers_dim ) t
...
FROM ecommerce.customers_dim -- Data spans from 2021-01-01 to 2023-12-31 , 3 full years 2021-2022-2023

--#Check age 

SELECT min(age) AS min_age, max(age) AS max_age, avg(age) AS avg_age
FROM ecommerce.customers_dim  -- min: 18, max: 70, avg: 35
...
FROM ecommerce.customers_dim -- 7 countries -> Possible standardization later on to full names for better understanding and business logic

SELECT country, COUNT(*) AS country_records
FROM ecommerce.customers_dim
GROUP BY country
...
-------------- ecommerce.products_dim | First Exploration

--Check top 10 col - table inspection
SELECT TOP 10 * FROM ecommerce.products_dim

--#Check total records/rows
SELECT COUNT(*) FROM ecommerce.products_dim -- 2000 rows.
...
FROM ecommerce.products_dim   -- no missing data
...
SELECT COUNT(DISTINCT(brand)) FROM ecommerce.products_dim -- 100
...
SELECT COUNT(DISTINCT(category)) FROM ecommerce.products_dim -- 6
...
-------------- ecommerce.transaction_fact | First Exploration

--#check top 10 rows
SELECT TOP 10 * FROM ecommerce.transaction_fact -- I already see a missing campaing_id VALUE

--#check total numbers of records
SELECT COUNT(*) FROM ecommerce.transaction_fact --- 103.127
...
FROM ecommerce.transaction_fact    ---------------- We notice that we have 10.449 missing values for "product_id", 10.449 for "gross_revenue" [We checked and are on the same exact records]
                                   

--#Lets check the first and last transaction data point we have 

SELECT MIN(transaction_timestamp) AS first_transaction , MAX(transaction_timestamp) FROM ecommerce.transaction_fact -- F: 2021-01-01 00:12 and L: 2023-12-31 22:37


--# Count the customers that proceed to a transaction and their records

SELECT COUNT(DISTINCT customer_id) FROM ecommerce.transaction_fact -- 64035 UNIQUE customer_ids


SELECT customer_id, COUNT(*) AS customers_transactions
FROM ecommerce.transaction_fact
...
SELECT COUNT(DISTINCT product_id) FROM ecommerce.transaction_fact -- 2000 = All products were sold at least once
...
SELECT COUNT (DISTINCT campaign_id) FROM ecommerce.transaction_fact --50 we have all campaigns
...
FROM ecommerce.transaction_fact  -- Min: -873.04 (transaction canceled / product returned), Max: 1858.32 and AVG: 90.36
...
-------------- ecommerce.events_fact | First Exploration --------- Event:  Customer Online Website Session 

--#Quick inspections top 10 rows
SELECT TOP 10 * FROM ecommerce.events_fact

--#Check number of records
SELECT COUNT(*) FROM ecommerce.events_fact -- Sth more than 1 million.

--#Check Uniqur Event_IDs

SELECT event_id, COUNT(*) count_events
FROM ecommerce.events_fact
GROUP BY event_id
...
SELECT MIN(events_timestamp) AS first_event , MAX(events_timestamp)  AS last_event FROM ecommerce.events_fact -- F: 21-01-01 00:01 and L: 2023-12-31 23:57


--#Check number of customers from the events table
SELECT count(DISTINCT customer_id) FROM ecommerce.events_fact --# 99.998  Unique customers In contrast to 100k from customers_dim table
...
--#Unique session_ids
SELECT COUNT(DISTINCT session_id) AS count_sessions FROM ecommerce.events_fact --#528098
...
SELECT COUNT(DISTINCT product_id) FROM ecommerce.events_fact -- all products are included in the events table
...
SELECT MIN(session_duration_sec) AS min_sess_dur_seconds , MAX(session_duration_sec)/60 AS max_sess_dur_minutes, AVG(session_duration_sec)/60 AS avg_sess_dur_minutes FROM ecommerce.events_fact 
...
--Here I Am gonna clean every table seperataly and will make any necessary changes.

-- I am gonna establish all possible dynamic table replationships so I can merge the tables and create the gold views OF MY projects.




SELECT * FROM ecommerce.products_dim
SELECT * FROM ecommerce.transaction_fact
